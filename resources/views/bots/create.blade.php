...
<div class="px-4 py-6 sm:px-0">
    ...
    <h1 class="text-2xl font-bold text-gray-900 mb-6">{{ __('Crear Bot') }}</h1>
        ...
        <form action="{{ route('bots.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required
                       ...
                       placeholder="{{ __('Ej: Asistente de ventas') }}">
            </div>
            ...
            <div>
                ...
                @if(!empty($channels))
                    <select name="openline_id" id="openline_id"
                        ...
                        @foreach($channels as $channel)
                            <option value="{{ $channel['ID'] }}" {{ old('openline_id') == $channel['ID'] ? 'selected' : '' }}>
                ...
                @else
                    <input type="text" name="openline_id" id="openline_id" value="{{ old('openline_id') }}"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                           placeholder="{{ __('ID del Open Channel') }}">
                    <p class="mt-1 text-xs text-yellow-600">{{ __('No se pudieron obtener los canales desde Bitrix24. Escribe el ID manualmente.') }}</p>
                @endif
            </div>

            @php $selectedLine = old('openline_id'); @endphp
                ...
                <div class="flex items-center justify-between">
                    ...
                    <label class="inline-flex items-center space-x-2 text-sm text-gray-700">
                        <input type="checkbox" name="menu_enabled" value="1" id="menu_enabled"
                               ...
                               {{ old('menu_enabled') ? 'checked' : '' }}>
                        <span>{{ __('Activar menu') }}</span>
                    </label>
                </div>

                <div id="menu-panel" class="mt-4 {{ old('menu_enabled') ? '' : 'hidden' }}">
                    <div>
                        ...
                        <textarea name="menu_greeting" id="menu_greeting" rows="2"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                  placeholder="{{ __('Hola, para poder ayudarte mejor responde solo con el numero de una de las siguientes opciones:') }}">{{ old('menu_greeting') }}</textarea>
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <table class="mt-2 min-w-full divide-y divide-gray-200">
                            ...
                            <tbody id="menu-options-body" class="bg-white divide-y divide-gray-200">
                                @forelse(old('menu_options', []) as $index => $row)
                                    <tr>
                                        <td class="px-3 py-2">
                                        ...
                                        <td class="px-3 py-2">
                                            <select name="menu_options[{{ $index }}][entity_id]"
                                                ...
                                                @include('bots.partials.menu-destination-options', [
                                                    'lineQueues' => $channelQueues[$selectedLine] ?? [],
            ...
            <div>
                ...
                <textarea name="system_prompt" id="system_prompt" rows="6"
                          class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                          placeholder="{{ __('Eres un asistente de ventas. Debes responder de forma clara y profesional.&#10;Cuando no tengas suficiente informacion debes solicitar mas detalles.&#10;Si el cliente solicita hablar con una persona debes transferir la conversacion a un operador.') }}">{{ old('system_prompt') }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="status" id="status"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    <option value="active" {{ old('status', 'active') === 'active' ? 'selected' : '' }}>{{ __('Activo') }}</option>
                    <option value="inactive" {{ old('status') === 'inactive' ? 'selected' : '' }}>{{ __('Inactivo') }}</option>
                </select>
            </div>
...
<script>
    ...
    let menuRowIndex = @json(count(old('menu_options', [])));
